...
import sqlite3
from utils.search_images import img_embedding
import pickle
from config import DB_PATH, IMAGES_DIR, PREVIEW_DIR, EMBEDS_DIR
...
def get_preview_list(sort='date_create', order='DESC'):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(f'SELECT id, preview_path, embed_path, strftime("%d.%m.%Y %H:%M:%f", date_create), size FROM images ORDER BY {sort} {order};')
    images = cur.fetchall()
    cur.close()
    conn.close()
    return images
...
def insert_image(image_path):
    ...
    unique_embed_name = f'embed_{uuid.uuid4()}.pkl'
    
    dest_path = os.path.join(IMAGES_DIR, unique_name)
    preview_path = os.path.join(PREVIEW_DIR, unique_preview_name)
    embed_path = os.path.join(EMBEDS_DIR, unique_embed_name)
    ...
    pickle.dump(img_embedding(dest_path), open(embed_path, 'wb'))
    
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO images (image_path, preview_path, embed_path, size)
        VALUES (?, ?, ?, ?)
    ''', (dest_path, preview_path, embed_path, size))
    conn.commit()
    cur.close()
    conn.close()
